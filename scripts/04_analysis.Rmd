...
bec_prot_donut %>%
  ...
  geom_hline(aes(yintercept = 0.162, colour = "Total BC Proportion Protected (16.2%)"), lty = "dotted") +
...
ggsave(here::here("outputs", "test.png"), device = "png")
...
```{r}
stat.test <- subzone_formatted %>%
  group_by(subzone, name) %>%
  rstatix::t_test(value ~ protected) %>%
  mutate(p.adj = p * 496) %>% # bonferroni correction
  add_significance()

stat.test <- stat.test %>% add_xy_position(x = "name")

ggboxplot(subzone_formatted, x = "protected", y = "value",
          facet = c("var_long", "value"))
```
...
names <- c("CDF_mm", "ESSF_wvp")

df <- map_df(here::here("data", "all_vars", "sampled", paste0(names, "-2015.csv")), read_csv)
...
stat.test <- df_formatted %>%
  group_by(subzone, name) %>%
  rstatix::t_test(value ~ protected) %>%
  adjust_pvalue(method = "bonferroni") %>%
  #mutate(p.adj = p * 496,
  #       p.adj = ifelse(p.adj > 1, 1, p.adj)) %>% # bonferroni correction
  add_significance() 

stat.test <- stat.test %>%
  add_xy_position(x = "protected")
  

box = ggboxplot(
  df_formatted, x = "protected", y = "value",
  facet.by = c("subzone", "var_long"),
...
ggsave(here::here("outputs", "bec_map.png"), device = "png")
```
