...
t_tests_scatter <- t_tests_zonal %>%
  ...
  labs(x = "Percent of Significant (P < 0.01) Tests",
       y = NULL) +
  scale_colour_igv() +
  labs(colour = NULL) +
...
lat_df <- lat_dist %>% 
  ...
  mutate(latitude = latitude - min(latitude),
         per_lat = latitude / max(latitude),
         lat_real = (per_lat * lat_range) + lat_bbox$ymin)

dist_colours <-  pal_igv("default")(3)[c(2, 3, 1)]

lat_plot <- lat_df %>%
  ggplot(aes(x = lat_real, y = perc, fill = Change_Attribution)) +
  geom_area() +
  facet_wrap(~str_to_title(protected)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L)) +
  coord_flip(ylim = c(.75, 1)) +
  labs(x = expression("Latitude " ( degree)),
       y = "Percent Disturbed",
       fill = "Disturbance") +
  theme(legend.position = "bottom") +
