...
The protected area system in British Columbia is designed to serve as areas available for ecological conservation as well as human recreation [@bcparks2012]. The system consists of a network of PAs and PA complexes (multiple nearby PAs which share the same conservation goals), with large variations in size, ranging from `r min_area` to `r as.character(round(max_area, 0))` ha (Figure \@ref(fig:study-area)). Both the British Columbian [@bcparks2012] and Canada-wide protected areas mandates [@governmentofcanada2019] commit to conserving ecological integrity across the PA network, using the Canadian definition outlined in the introduction. Protected areas in the British Columbian network are frequently placed in mountainous, high elevation areas, leading to underrepresentation in high productivity ecosystems [@environmentalreportingbc2016].
...
Boundaries for BEC zones and subzones were acquired using the **bcmaps** R package [@R-bcmaps]. Two BEC subzones were entirely subsumed by PAs (Boreal White and Black Spruce - Very Wet Cool and Spruce -- Willow -- Birch - Very Wet Cool Shrub), whereas the Sub-Boreal Pine -- Spruce - Moist Cool subzone has no PAs associated with it. Due to this, these subzones were excluded from hypothesis testing, but were included in zonal aggregations. In this paper, zones are used to examine categorical data (land cover and disturbance), while forest structure is compared at the subzone level between PAs and their unprotected counterparts in British Columbia.
...
The Advanced Spaceborne Thermal Emission and Reflection Radiometer (ASTER) digital elevation model (GDEM V2) was used to examine biases in protected area land cover and ecological classification by elevation. This DEM was created by using stereo-paired images from the ASTER instrument aboard the TERRA satellite, and is available globally at a 30m resolution [@tachikawa2011].
...
As elevation increases in British Columbia, more and more terrestrial area is protected until \~4000m, where all terrestrial area in BC becomes protected (Figure \@ref(fig:bec-elev)). When comparing between protected and unprotected areas, some ecosystem zones are protected at differing rates. Zones commonly found at high elevations, such as the Boreal Altai Fesuce Alpine, are more common in protected areas than unprotected areas. The Coastal Western-hemlock ecosystem takes up more of the protected areas at low elevations, while Boreal White and Black Spruce takes up this area in unprotected areas. Other ecosystems are found at similar rates in both protected and unprotected areas (Figure \@ref(fig:bec-elev)).
...
Land cover classes also vary in their proportion of the landscape protected (Figure \@ref(fig:vlce-conch)). Snow/ice, exposed/barren land, and rock/rubble have higher than average proportions of their terrestrial area protected. Mixedwood and broadleaf land cover classes are underrepresented in BC's PA network. All other classes are found at rates similar to the overall proportion of the province protected (Figure \@ref(fig:vlce-conch)).
...
Similar to BEC zones (Figure \@ref(fig:bec-elev), land cover classes also vary with elevation (Figure \@ref(fig:lcc-elev). Snow and ice make up a large proportion of PAs at high altitudes. The only water present in high elevations is found in lakes in protected areas, mostly above 3500m. At lower elevations in unprotected areas, mixedwood forest is a more common forest type than in PAs. Wetland classes are found less frequently in the 400-900m range in unprotected areas than in PAs.
...
Disturbance regimes in British Columbia since 1984 in protected and unportected areas are shown in Figure \@ref(fig:lat-dist). Fire regimes are similar between both protected and unprotected strata at all latitudes. Harvesting is not common in PAs, while it is frequently found in unprotected areas. Within unprotected areas, harvesting is more common at lower latitudes.
...
Figure \@ref(fig:t-test-plot) shows the subzonal proportional significance (P \< 0.01) by zone of the 496 t-tests by forest structural variable. Higher percentages indicate that a zone has more dissimilar subzones than lower percentages. At least half of all subzones in each zone are significantly different (exception being ponderosa pine, which has one subzone which is not significantly different in canopy structure). The median values for canopy height, canopy cover, and aboveground biomass are all 100% significantly different.

# Discussion

Protected areas conservation is an effective means of conserving biodiversity and valuable ecosystem services. Currently, many nations and international governments use proportional area protected as a critical metric for assessing progress towards biodiversity conservation. However, these metrics do not assess the effectiveness of protected area networks, nor do they assess how representative PAs are at protecting the ecosystems they aim to protect. To face these challenges, we examined the protected area network of British Columbia using satellite remote sensing data.
